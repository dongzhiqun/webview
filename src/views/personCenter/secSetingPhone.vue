  ...
  <div class="edit-pass-box">
    <div class="header-breadcrumb">
        ...
        <div class="edit-pass-header">
            <p class="edit-pass-title">手机绑定</p>
        ...
        <div class="edit-pass-body">
            <div class="edit-pass-sure" v-if="progessNow == '0'">
                ...
                <secMain :type='type' @new-phone="curenntPhone" @new-user="nowUser"></secMain>
                <p class="edit-sign">
                    没有收到短信验证码？<br>
            ...
            <div class="edit-pass-edit" v-if="progessNow == '1'">
                <p>我们将不会泄露您的手机信息</p>
                <el-form ref="codeForm1" :model="codeForm1" :rules="rules1" label-width="80px">
                <el-form-item label="手机号" prop="newPhone">
                  <el-input v-model="codeForm1.newPhone"></el-input>
                </el-form-item>
                <el-form-item label="验证码" prop="verificationCode">
                  <el-input v-model="codeForm1.verificationCode" style="width:60%"></el-input>
                  <sendCode
                    ref="regSendCode"
                    :phone='codeForm1.newPhone'
                  ...
                  <el-button type="primary"
                                  @click="goEditPass('codeForm1')" class="go-back-button">下一步</el-button>
                                  </div>
                </el-form>
            </div>
            <!-- 修改成功 -->
            <div class="edit-psaa-success" v-if="progessNow == '2'">
                ...
                <p class="success-word">手机号码修改/设置成功</p>
...
import { saveNewPhone } from '@/api/userInfo.js'
import { getTokenCode } from '@/api/user.js'
export default {
  name: 'edit-pass',
  components: { secProgess, secMain, sendCode, backButton },
  data () {
  ...
  methods: {
    ...
    // 新手机修改
    goEditPass (codeForm1) {
      this.$refs[codeForm1].validate((valid) => {
        if (valid) {
          this.codeForm1.oldPhone =
          saveNewPhone(this.codeForm1).then((res) => {
            if (res.code === 0) {
              this.$message({
                message: '修改成功',
                type: 'success'
    ...
    curenntPhone (data) {
      this.codeForm1.oldPhone = data
      this.progessNow = '1'
    },
    // 倒计时
    countDownMethod () {
      // 获取短信验证码
      const params = { phone: this.codeForm1.newPhone, type: '21' }
      getTokenCode(params).then((res) => {
        if (res.code === 0) {
          this.$message({
            message: '已向您的手机号' + this.codeForm1.newPhone + '发送验证码',
            type: 'success'
          })
