...
export default {
    ...
    methods: {
        listAllBlog () {
            ...
            this.$http.get('http://172.20.10.4:8876/blog/getAllBlogs').then(result => {
                // 注意： 通过 $http 获取到的数据，都在 resultresult.body 中放着
        ...
        deleteBlog (index, row) {
            this.$http.get('http://172.20.10.4:8876/blog/deleteBlogById?id=' + row.id).then(result => {
        ...
        search () {
            this.$http.get('http://172.20.10.4:8876/blog/getBlogById?id=' + this.input).then(result => {
                console.log(result)
                var result = result.data
                if (result.code === 200) {
                    console.log(result.data)
